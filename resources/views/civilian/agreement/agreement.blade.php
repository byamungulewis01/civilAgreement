...
<div class="row g-4">
    ...
    $national_id = ($item->partyOne == auth()->guard('civilian')->user()->id) ? $item->party2->national_id :
    $item->party1->national_id;
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-start">
                    <div class="d-flex align-items-start">
                        <div class="me-2 ms-1">
                            <h5 class="mb-0"><a href="{{ route('civilian.agreement.show',$item->id) }}"
                                    class="stretched-link text-body">{{ $name }}</a></h5>
                            <div class="client-info"><span class="text-muted">{{ $national_id }}</span></div>
                        </div>
                    </div>
                    <div class="ms-auto">
                        @if ($item->status == 'pending')
                        <span class="badge bg-label-primary ms-auto">Pending</span>
                        @elseif($item->status == 'rejected')
                        <span class="badge bg-label-danger ms-auto">Rejected</span>
                        @elseif($item->status == 'accepted')
                        <span class="badge bg-label-secondary ms-auto">Accepted</span>
                        @elseif($item->status == 'completed')
                        <span class="badge bg-label-success ms-auto">Completed</span>
                        @else
                        <span class="badge bg-label-danger ms-auto">Fail</span>
                        @endif
                    </div>
                </div>
            </div>
